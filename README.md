...
<p>このリポジトリは、awkコマンドを実行して三角関数のsinとcosを使用して指定した値から結果を出力します</p>
</div>
...
<div class="wp-block-group">
<p>awkコマンドを実行してsinとcosの値を出力してみてください</p>
...
<pre class="wp-block-code has-background" style="background-color:#ffeeee"><code>sin:0
cos:1
sin:1
cos:0.000796327
sin:0
...
<pre class="wp-block-code has-24292-eff-color has-text-color has-background has-1-125-rem-font-size" style="background-color:#ffeeee"><code>sin:0
cos:1
sin:1
cos:0.000796327
sin:0
...
<div class="wp-block-group">
<p>sinやcosに指定する値は度数ではなく弧度表記にする必要があります<br>tanは以下のコードのようにsin/cosを計算することで取得できます</p>



<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>echo 0.785 | awk '{print "tan:"sin($1)/cos($1)}'</code></pre>
</div>
